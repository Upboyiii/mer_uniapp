	...
	methods: {
		...
		confirmPayInSheet() {
			...
			const payPayload = {
				...
				payType,
				// 给后端/回跳页一个明确来源标识
				from: this.isTcm ? 'clinic_appointment_tcm' : 'clinic_appointment_physio'
			};
				...
				.then((payRes) => {
					...
					if (d.alipayRequest) {
						...
						try {
							...
							uni.setStorageSync('clinicAppointmentPayAppointmentId', String(this.detail.id));
						} catch (e) {}
						this.handleOrderPay(
							payRes,
							...
							undefined,
							{ appointmentId: this.detail.id, category: cat }
						);
						return;
					}
					{
